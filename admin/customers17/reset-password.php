...
try {
    $stmt = $conn->prepare("SELECT CustomerID, Name, CustomerUniqueID FROM mp_customers WHERE CustomerID = ?");
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        if (empty($newPassword) || empty($confirmPassword)) {
            ...
            try {
                ...
                // Update the password
                $stmt = $conn->prepare("UPDATE mp_customers SET PasswordHash = ? WHERE CustomerID = ?");
